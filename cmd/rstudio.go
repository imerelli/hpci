...
func init() {
	...
	rstudioCmd.Flags().StringVarP(&cfgRstudioFile, "file", "f", "", "config file")
	...
	rstudioCmd.Flags().StringSliceVarP(&RstudioCfg.Directory,"directory", "d", []string{}, "add extra directory (absolute path separate with comma or repeat flag)")
